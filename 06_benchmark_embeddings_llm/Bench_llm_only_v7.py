...
MAX_EVIDENCES = int(
    os.environ.get("BENCH_MAX_EVIDENCES", "10")
)  # evidence lines in prompt
...
- Écris EXACTEMENT {top_recos} lignes.
- Chaque ligne DOIT commencer par "1) ", "2) ", "3) " (dans cet ordre, sans sauter de numéro).
- 1 seule phrase par ligne.
- EXACTEMENT 1 citation par ligne, choisie dans "CITATIONS AUTORISÉES".
- La citation doit être le DERNIER élément de la ligne : la ligne doit se terminer par ] (idéalement sans point).
- Aucun texte en dehors de ces lignes (pas de titre, pas de markdown, pas de puces).
...
- Français uniquement.
- Écris EXACTEMENT {top_recos} lignes.
- Chaque ligne DOIT commencer par "1) ", "2) ", "3) " (dans cet ordre).
- 1 seule phrase par ligne.
- EXACTEMENT 1 citation par ligne, prise dans la liste "CITATIONS AUTORISÉES".
- La citation est le DERNIER élément de la ligne : la ligne doit se terminer par ] (idéalement sans point).
- Aucun texte hors de ces lignes.
- Interdit : mettre deux citations sur la même ligne.

Voici ta réponse invalide (à corriger) :
{bad_response}
...
def lang_is_frenchish(text: str) -> bool:
    ...
    if not toks:
        return True
    fr = sum(1 for t in toks if t in FR_STOP) + sum(
        1 for ch in text if ch in "éèêëàâäùûüîïôöç"
    )
...
def load_retrieval_assets(conn, run_id: str):
    ...
    if not row:
        raise RuntimeError(
            f"Retrieval run_id not found or missing faiss index: {run_id!r}"
        )
...
def load_queries(conn, split: str) -> list[QueryRow]:
    ...
    return [
        QueryRow(int(qid), str(intent), str(qtext)) for (qid, intent, qtext) in rows
    ]


def build_doc_key_to_entity(conn) -> dict[str, Entity]:
    # From corpus_docs: doc_key -> entity (prefer series_id, else kitsu_id)
    rows = fetch_all(conn, "SELECT doc_key, series_id, kitsu_id FROM bench.corpus_docs")
    mp: dict[str, Entity] = {}
...
def load_qrels_entities(
    conn, split: str, doc_key_to_entity: dict[str, Entity]
) -> dict[int, set[Entity]]:
...
def ollama_generate(model: str, system: str, prompt: str, options: dict) -> str:
    ...
    /api/chat reduces prompt-continuation issues for some instruct models (notably Phi).
    """
...
def citations_validity(
    end_cits: list[tuple[str, int]], chunkid_to_dockey: dict[int, str]
) -> float:
    if not end_cits:
        return 0.0
    ok = 0
...
def cited_entities_from_end_citations(
    end_cits: list[tuple[str, int]], doc_key_to_entity: dict[str, Entity]
) -> list[Entity]:
    ents = []
...
def score_entity_metrics(
    rec_entities: list[Entity], rel_entities: set[Entity]
) -> tuple[float, float]:
...
def main():
    if not RETRIEVAL_RUN_ID:
        raise SystemExit(
            "ERROR: BENCH_RETRIEVAL_RUN_ID is required (uuid from bench.embedding_runs)."
        )
    ...
    with psycopg.connect(DB_DSN) as conn:
        ...
        embed_model_name, index_path, meta_path, index, chunk_id_arr, doc_key_arr = (
            load_retrieval_assets(conn, RETRIEVAL_RUN_ID)
        )
        ...
        print(
            f"[INFO] num_ctx={OLLAMA_OPTIONS['num_ctx']} | num_predict={OLLAMA_OPTIONS['num_predict']}"
        )
        ...
        chunkid_to_dockey = {
            int(chunk_id_arr[i]): str(doc_key_arr[i]) for i in range(len(chunk_id_arr))
        }
        ...
        print(
            f"[INFO] queries={len(queries)} | qrels_entities(queries_with_rel)={len(qrels_entities)}"
        )

        out_path = os.path.join(OUT_DIR, "llm_results_v7.jsonl")
        all_rows = []
        ...
        for model in LLM_MODELS:
            ...
            for q in queries:
                ...
                retrieved_chunk_ids = []
                retrieved_scored = []
                for ix, sc in zip(idxs, scores, strict=False):
                    if ix < 0:
                        continue
                ...
                evids = select_evidences(
                    q.query_text, candidates, rel_ents, MAX_EVIDENCES
                )
                ...
                resp = ollama_generate(
                    model, SYSTEM_PROMPT, user_prompt, OLLAMA_OPTIONS
                )
                ...
                cit_valid = (
                    citations_validity(end_cits, chunkid_to_dockey) if end_cits else 0.0
                )
                cit_vals.append(cit_valid)

                # V4 scoring: entities derived from citations
                rec_entities = (
                    cited_entities_from_end_citations(end_cits, doc_key_to_entity)
                    if end_cits
                    else []
                )
                ...
                print(
                    f"[{status}] {model} qid={q.query_id} intent={q.intent} latency={dt:.2f}s hit@{TOP_RECOS}={hit:.2f} recall@{TOP_RECOS}={recall:.2f} cit_valid={cit_valid:.2f} retry={retry}"
                )
                ...
                row = {
                    ...
                    "response": resp,
                    "end_citations": [
                        {"doc_key": dk, "chunk_id": cid} for (dk, cid) in end_cits
                    ],
            ...
            print(
                f"[SUMMARY] {model} | n={n} | lat(mean/med/p95)={mean_lat:.2f}/{med_lat:.2f}/{p95_lat:.2f}s "
