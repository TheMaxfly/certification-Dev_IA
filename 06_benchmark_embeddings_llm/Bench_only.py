...
def entity_key(series_id: int | None, kitsu_id: int | None) -> str | None:
...
def load_dockey_to_entity(conn) -> dict[str, str]:
    ...
    if DEBUG:
        print(
            f"[DEBUG] doc_key->entity mapping size={len(m)} | missing(no series_id/kitsu_id)={missing}"
        )
    return m
...
def load_qrels_entities(conn, dockey_to_entity: dict[str, str]) -> dict[int, set[str]]:
    ...
    if DEBUG:
        print(
            f"[DEBUG] qrels entity size (queries)={len(qrels_ent)} | missing doc_key in map={missing}"
        )
    return qrels_ent
...
def list_known_faiss_run_ids(conn) -> list[str]:
    rows = fetch_all(
        conn, "SELECT run_id::text FROM bench.faiss_indexes ORDER BY built_at DESC"
    )
...
def load_index_and_meta(conn, run_id: str):
    ...
    if not row2:
        raise RuntimeError(
            f"run_id={run_id!r} exists in faiss_indexes but not in embedding_runs/models join."
        )
    model_name = row2[0]
...
def evaluate_run_entity_only(conn, run_id: str):
    model_name, index, chunk_id_arr, doc_key_arr, index_path, meta_path = (
        load_index_and_meta(conn, run_id)
    )
    ...
    if len(qrels_ent) == 0:
        raise RuntimeError(
            "No qrels in bench.qrels (or none could be mapped to entity keys)."
        )
    ...
    for i, row in enumerate(queries.itertuples(index=False), start=1):
        ...
        q_emb = st_model.encode(
            [qtext], normalize_embeddings=True, show_progress_bar=False
        )
        ...
        if DEBUG and i <= 2:
            ...
            print(f"[DEBUG] rel_entities={sorted(rel_set)[:12]}")
    ...
    print(
        f"[RESULT] queries={len(queries)} | entity_recall@{TOP_K}={recall_at_k:.3f} | entity_mrr={mrr:.3f}"
    )
...
def main():
    ...
    with psycopg.connect(DB_DSN) as conn:
        ...
        if DEBUG:
            known = list_known_faiss_run_ids(conn)
            print(
                "[DEBUG] Known FAISS run_ids in DB (bench.faiss_indexes):", known[:10]
            )
